...
public class ProviderOptimizerService
{
    ...
    /// <summary>
    /// Obtiene los proveedores disponibles ordenados por distancia al cliente (más cercano primero),
    /// con la distancia en km. Sirve para devolver lista con ETA.
    /// </summary>
    public List<(Provider Provider, double DistanceKm)> GetOrderedProvidersWithDistance(
        List<Provider> proveedores,
        double latCliente,
        double lngCliente)
    {
        var disponibles = proveedores.Where(p => p.Disponible).ToList();
        if (disponibles.Count == 0)
            return new List<(Provider, double)>();

        return disponibles
            .Select(p => (
                p,
                DistanceKm: CalculateDistance(latCliente, lngCliente, p.Latitud, p.Longitud)
            ))
            .OrderBy(x => x.DistanceKm)
            .ThenByDescending(x => x.p.Calificacion)
            .ToList();
    }
